...
while running:
    ...
    # Render text
    spin_text = font.render(spin_direction, True, WHITE)
    velocity_text = font.render(f"Ball Speed: {ball_speed:.2f}", True, WHITE)
    bounce_text = font.render(f"Bounce Angle: {bounce_angle:.2f}°", True, WHITE)
    ...
    pitch_text = font.render(f"Pitch (X): {x_direction}{angle_x_deg:.2f}°", True, WHITE)
    yaw_text = font.render(f"Yaw (Y): {y_direction}{angle_y_deg:.2f}°", True, WHITE)
    roll_text = font.render(f"Roll (Z): {z_direction}{angle_z_deg:.2f}°", True, WHITE)
